...
- Control Panel menuntut sesi operator (email + password). Buat akun pertama setelah migrasi: `docker exec -it jawara-gateway python -m app.scripts.create_operator --email ops@instansi.id --name "Nama"`. Tanpa itu tidak ada yang bisa masuk — dan itu memang perilaku yang benar, bukan akun default. Umur sesi diatur `AUTH_SESSION_TTL_MINUTES`. **RBAC belum ada**: setiap akun melihat seluruh panel ([[Implement_Operator_Auth]]).
